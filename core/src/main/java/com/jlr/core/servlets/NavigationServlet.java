...
/**
 * Navigation Servlet is used to fetch the header nav based on request
 * parameters, and also to return the json.
 */
@Component(service = Servlet.class, property = { Constants.SERVICE_DESCRIPTION + "=Navigation Servlet",
        "sling.servlet.resourceTypes=" + RESOURCE_TYPES, "sling.servlet.selectors=" + NavigationServlet.SELECTOR_JSON })
...
public class NavigationServlet extends SlingSafeMethodsServlet {
    ...
    protected void doGet(final SlingHttpServletRequest request, final SlingHttpServletResponse response)
        ...
        Boolean search = request.getParameter("search") == null ? Boolean.TRUE
                : Boolean.valueOf(request.getParameter("search"));
        Boolean yourRetailer = request.getParameter("yourRetailer") == null ? Boolean.TRUE
                : Boolean.valueOf(request.getParameter("yourRetailer"));
        Boolean retailerLocatorLink = request.getParameter("retailerLocatorLink") == null ? Boolean.TRUE
                : Boolean.valueOf(request.getParameter("retailerLocatorLink"));
        ...
        if (locale.equalsIgnoreCase("en_AU") && !requestPath
                .contains(CommonConstants.FORWARD_SLASH + locale.toLowerCase() + CommonConstants.FORWARD_SLASH)) {
        ...
        if (Boolean.FALSE.equals(mrp)) {
            req.setAttribute(PricingConstants.PRICING_SUPPRESSION, "true");
        }
    ...
    private void sendResponse(SlingHttpServletResponse response, Boolean cache, Document document) throws IOException {
        /*
         * cache maxAge=750 or 15 min, retailer Url and retailer name substitute
         * with request param value
         */
